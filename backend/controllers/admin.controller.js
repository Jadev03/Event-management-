...
const getMonthlyLoginTrafficAnalytics = async (req, res) => {
  try {
    const monthsBackRaw = Number.parseInt(req.query?.months || '6', 10);
    const monthsBack = Number.isFinite(monthsBackRaw)
      ? Math.min(Math.max(monthsBackRaw, 1), 24)
    ...
    const agg = await LoginAttempt.aggregate([
      { $match: { attemptedAt: { $gte: start, $lt: endExclusive } } },
      {
        $group: {
          _id: {
            y: { $year: '$attemptedAt' },
    ...
    for (const row of agg || []) {
      ...
      const failedAttempts = row?.failedAttempts || 0;
      if (!y || !m1) continue;
      ...
      cur.totalAttempts += totalAttempts;
      cur.failedAttempts += failedAttempts;
      cur.trafficUsers = Math.max(cur.trafficUsers, row?.trafficUsers || 0);
    ...
    return res.status(200).json({ months: monthsBack, series });
  } catch (error) {
    logger.error('Error building monthly login traffic analytics', {
      message: error.message,
    });
    return res.status(500).json({ message: 'Internal server error' });
  }
